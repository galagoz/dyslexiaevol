...
outputdir = "/data/clusterfs/lag/users/gokala/dyslexia-evol/sds"
...
output=data.frame(global_corr_spearman=rep(NA, 1),
               BJK_ESTIM_AVE=rep(NA, 1),
               BJK_ESTIM_SE=rep(NA, 1),
               BJK_ESTIM_Z=rep(NA, 1),
               BJK_ESTIM_PVAL=rep(NA, 1));
...
##Loop over each of the phenotypes
    ...
    load("/data/clusterfs/lag/users/gokala/dyslexia-evol/ancreg_sumstat/Dyslexia_ancreg.Rdata")
    ...
    mcols(newmergedGR) = merged[,c(1,4:17,25)] # added 25th column (Z) here, otherwise "dat" didn't have newmergedGR$Z for ancestry regressed sumstats, and 19th column for non-ancreg sumstats (ENIGMA-replication sumstats)
   ...
   output$global_corr_spearman[1]=global_corr_spearman
   output$BJK_ESTIM_AVE[1]=BJK_ESTIM_AVE;
   output$BJK_ESTIM_SE[1]=BJK_ESTIM_SE;
   output$BJK_ESTIM_Z[1]=BJK_ESTIM_Z;
   output$BJK_ESTIM_PVAL[1]=BJK_ESTIM_PVAL;

   rownames(output)[1]=pheno;
   write.csv(output, file=paste0(outputdir, "/SDS_bjk_ancreg_dyslexia.csv"))